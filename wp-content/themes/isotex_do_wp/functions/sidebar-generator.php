<?php
	...
	function admin_menu(){
		add_theme_page('Sidebars', 'Sidebars', 'manage_options', 'multiple_sidebars', 'admin_page');
		...
		<div class="wrap">
			...
			<table class="widefat page" id="sbg_table" style="width:600px;">
				...
				<tr class="<?php echo $alt?>">
					...
					<td><?php echo name_to_class($sidebar); ?></td>
		...
		<div id="sbg_box" class="postbox " >
			...
			<div class="inside">
				<div class="sbg_container">
					...
					<p>Please select the sidebar you would like to display on this page.</p>
					<ul>
					<?php 
					...
					//var_dump($wp_registered_sidebars);		
						for($i=0;$i<1;$i++){ ?>
							...
							<?php
							$sidebars = $wp_registered_sidebars;// get_sidebars();
							if(is_array($sidebars) && !empty($sidebars)){
								foreach($sidebars as $sidebar){
							...
							<select name="sidebar_generator_replacement[<?php echo $i?>]">
								<option value="0"<?php if($selected_sidebar_replacement[$i] == ''){ echo " selected";} ?>>Global Sidebar</option>
							<?php
							
							$sidebar_replacements = $wp_registered_sidebars;//get_sidebars();
