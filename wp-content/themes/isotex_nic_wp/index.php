<?php 
	...
	$archive_column = rd_options('reedwan_archive_column');
	$archive_sidebar = rd_options('reedwan_archive_sidebar');
	$archive_masonry = rd_options('reedwan_archive_masonry');
	$archive_share = rd_options('reedwan_archive_share');
	$archive_post_meta = rd_options('reedwan_archive_meta');
	$archive_post_more = rd_options('reedwan_archive_readmore');
...
?>
<div class="page-content <?php echo $archive_column; ?> <?php echo $archive_sidebar; ?>">
		...
		<div class="row_inner">
			<div id="post-<?php the_ID(); ?>" <?php post_class('posts pages_cont'.$grids_wrap.' archives '.$sidebar); ?>>
				<ul class="row_inner grid_layout"<?php echo $isotope; ?>>
					...
					?>
					<li id="post-<?php the_ID(); ?>" class="grid_item <?php echo $grid; ?>">
						<div class="post_b">
								...
								<div class="post_inner_content clearfix">
									...
									<?php if($archive_post_meta==1 ): ?>
										<div class="f_meta clearfix">
											<?php echo post_meta($disqus_shortname, $metaAuthor, $date, $comments, $views, $category);?>
										</div>
									<?php endif; ?>
									<?php echo excerpt($excerpt); ?>
									<?php if($archive_post_more==1 ): ?>
										<div class="right more"><a href="<?php the_permalink(); ?>"><?php _e('Leer más','corporative'); ?> &rarr;</a></div>
									<?php endif; ?>
								</div>
							</div>
						</div>
