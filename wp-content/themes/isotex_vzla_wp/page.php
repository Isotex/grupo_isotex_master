<?php 
	get_header();
	$page_sidebar = $sidebar = $page_comment = $disqus_shortname = '';
	if(get_field( 'page_sidebar')){$page_sidebar = get_field( 'page_sidebar'); }
	if(get_field( 'page_comment' )){$page_comment = get_field( 'page_comment' ); }
	
	if($page_sidebar == 'default' || empty($page_sidebar)){$page_sidebar = rd_options('reedwan_page_sidebar'); }
	if($page_comment == 'default' || empty($page_comment)){$page_comment = rd_options('reedwan_page_comment'); }
	
	if($page_sidebar == 'sidebar_none'){$sidebar='grid_12';}
	else{$sidebar='grid_9';}
	
	if($page_comment == 'comment_disqus'){$disqus_shortname = rd_options('reedwan_disqus_shortname');}
?>

<div class="page-content<?php if($page_sidebar == 'sidebar_left'){echo ' left-sidebar';} ?>">
	<?php if (have_posts()) : while (have_posts()) : the_post(); ?>
	<div class="row clearfix">
		<div class="row_inner">
			<div id="post-<?php the_ID(); ?>" <?php post_class('pages pages_cont '.$sidebar); ?>>
				<?php the_content(); ?>
				<?php wp_link_pages( array( 'before' => '<div class="page-links">' . __( 'Pages:', 'corporative' ), 'after' => '</div>' ) ); ?>
				<?php if($page_comment != 'comment_none'):?>
				<div class="single-comments mts clearfix">
					<?php if($page_comment == 'comment_disqus'):?>
						<?php echo disqus_embed($disqus_shortname);?>
					<?php else: ?>
						<?php comments_template(); ?>
					<?php endif; ?>
				</div>
				<?php endif; ?>
			</div>
			<?php if($page_sidebar != 'sidebar_none'): ?>
			<div class="sidebar grid_3">
				<?php get_sidebar();?>
			</div>
			<?php endif; ?>
		</div>
	</div>
	<?php endwhile; wp_reset_postdata(); endif; ?>
</div><!-- end page content -->
<?php get_footer(); ?>